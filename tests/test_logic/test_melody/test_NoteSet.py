...
@pytest.mark.parametrize(
    ...
    [
        (Note("C3"), NOTE_SETS.DiatonicModes.MAJOR, Interval(12), Interval(0),
         [Note(n) for n in ["C3", "D3", "E3", "F3", "G3", "A3", "B3", "C4"]]),
        (Note(60), NOTE_SETS.DiatonicModes.MAJOR, Interval("P0+"), Interval("P4"),
...
def test_random_sample():
    ...
    # sample size smaller than set, start note specified
    ...
    # sample size larger than set, each cycle gets same starting note
    ...
    # sample size larger than set, start note only first element of sample
